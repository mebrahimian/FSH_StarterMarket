            ...
            MonthlyActivitySummary? existingSummary =
                     await dbContext.MonthlyActivitySummaries
                       .SingleOrDefaultAsync(
                         ...
                         && x.PeriodEndDate == MonthCell.PeriodEndToDate, cancellationToken);
            if (existingSummary is null)
            {
                var summary = new MonthlyActivitySummary(
                    symbol: disclosure.Symbol,
                    periodEndDate: MonthCell.PeriodEndToDate!,
                    yearEndDate: MonthCell.YearEndToDate,
                    rt: disclosure.Rt,

                    periodAmount: monthlySalesAmount,
                    yearToDateAmount: yearToDateSalesAmount,

                    periodFormula: MonthCell.Formula,
                    periodAddress: MonthCell.Address,
                    periodRowSequence: MonthCell.RowSequence,
                ...
                await dbContext.MonthlyActivitySummaries.AddAsync(
                    summary,
                    cancellationToken);
            }
            else if (
